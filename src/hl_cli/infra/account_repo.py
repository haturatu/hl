...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    if "network" not in columns:
        conn.execute(
            "ALTER TABLE accounts ADD COLUMN network TEXT NOT NULL DEFAULT 'mainnet'"
        )
    conn.commit()
...
def get_account_by_alias(alias: str, network: str) -> Optional[Account]:
    conn = _conn()
    row = conn.execute(
        "SELECT * FROM accounts WHERE alias = ? AND network = ?", (alias, network)
    ).fetchone()
    conn.close()
...
def get_default_account(network: str) -> Optional[Account]:
    conn = _conn()
    row = conn.execute(
        "SELECT * FROM accounts WHERE network = ? AND is_default = 1 LIMIT 1",
        (network,),
    ).fetchone()
    conn.close()
    return _to_account(row) if row else None
...
def get_account_count(network: str) -> int:
    conn = _conn()
    count = conn.execute(
        "SELECT COUNT(*) AS c FROM accounts WHERE network = ?", (network,)
    ).fetchone()["c"]
    conn.close()
    return int(count)
...
def create_account(
    *,
    ...
    if should_be_default:
        conn.execute(
            "UPDATE accounts SET is_default = 0 WHERE network = ? AND is_default = 1",
            (network,),
        )
    conn.execute(
        """
...
def delete_account(alias: str, network: str) -> bool:
    ...
    conn.execute(
        "DELETE FROM accounts WHERE alias = ? AND network = ?", (alias, network)
    )
    if was_default:
        first = conn.execute(
        ...
        if first:
            conn.execute(
                "UPDATE accounts SET is_default = 1 WHERE id = ?", (first["id"],)
            )
    conn.commit()
    conn.close()
    return True
