...
def migrate_encrypted_account_fields(conn: sqlite3.Connection) -> None:
    ...
    for row in rows:
        ...
        if (
            ...
            and (
                api_wallet_private_key is None
                or str(api_wallet_private_key).startswith(_ENC_PREFIX)
            )
            and (
                api_wallet_public_key is None
                or str(api_wallet_public_key).startswith(_ENC_PREFIX)
            )
        ):
            continue
        conn.execute(
